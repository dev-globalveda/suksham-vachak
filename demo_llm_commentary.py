...
Usage:
    python demo_llm_commentary.py [--match-id MATCH_ID] [--persona PERSONA] [--moments N] [--rag] [--stats] [--tts PROVIDER]
...
Examples:
    ...
    python demo_llm_commentary.py --stats  # Enable player matchup stats
...
# Optional Stats imports (always available - uses stdlib sqlite3)
try:
    from suksham_vachak.stats import StatsConfig, create_engine
...
def print_context_summary(context) -> None:
    ...
    # Show matchup stats if available
    if context.narrative.matchup_context:
        print(f"\n  📈 Matchup: {context.narrative.matchup_context}")
...
def run_demo(  # noqa: C901
    ...
    # Initialize Stats engine (optional)
    stats_engine = None
    if use_stats:
        if not STATS_AVAILABLE:
            print("❌ Stats module not available")
            sys.exit(1)
        print("\n📊 Initializing Stats Engine...")
        try:
            stats_config = StatsConfig.default()
            stats_engine = create_engine(stats_config)
            print("   ✅ Stats engine initialized")
            print(f"   📁 Database: {stats_config.db_path}")
        except Exception as e:
            print(f"❌ Failed to initialize Stats: {e}")
            sys.exit(1)

    # Initialize context builder
    context_builder = ContextBuilder(info, rag_retriever=rag_retriever, stats_engine=stats_engine)
    ...
    print(f"   Stats enabled: {use_stats}")
    ...
    if engine.use_llm:
        ...
        if use_stats:
            print("      + Player vs Bowler matchup statistics!")
        if use_rag:
...
def main() -> None:
    ...
    parser.add_argument(
        "--stats",
        action="store_true",
        help="Enable player vs bowler matchup statistics",
    )
    parser.add_argument(
        "--tts",
        type=str,
